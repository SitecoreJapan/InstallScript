...
$params = @{
		...
		SitecoreIdentityServerZipPath = Resolve-Path -Path "..\Sitecore.IdentityServer.1.2.3.zip"
		...
		CommerceConnectModuleFullPath = Resolve-Path -Path "..\Sitecore.Commerce.Habitat.Images-1.0.0.zip"	
		CommercexProfilesModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce ExperienceProfile Core 11.2.83.zip"	
		CommercexAnalyticsModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce ExperienceAnalytics Core 11.2.83.zip"	
		CommerceMAModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce Marketing Automation Core 11.2.83.zip"	
		CommerceMAForAutomationEngineModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce Marketing Automation for AutomationEngine 11.2.83.zip"	
        CEConnectPackageFullPath = Resolve-Path -Path "..\Sitecore.Commerce.Engine.Connect.2.2.86.update"
        PowerShellExtensionsModuleFullPath = Resolve-Path -Path "..\assets\Sitecore PowerShell Extensions-4.7.2 for Sitecore 8.zip"
        SXAModuleFullPath = Resolve-Path -Path "..\assets\Sitecore Experience Accelerator 1.7.1 rev. 180604 for 9.0.zip"
        SXACommerceModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce Experience Accelerator 1.2.205.zip"
		SXAStorefrontModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce Experience Accelerator Storefront 1.2.205.zip"
        SXAStorefrontThemeModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce Experience Accelerator Storefront Themes 1.2.205.zip"
		SXAStorefrontCatalogModuleFullPath = Resolve-Path -Path "..\Sitecore Commerce Experience Accelerator Habitat Catalog 1.2.205.zip"
